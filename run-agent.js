...
function checkApiKey(config) {
  const resolvedKey = config.vllmApiKey || config.nvidiaApiKey;
  if (resolvedKey && resolvedKey !== "your-zhipu-api-key-here") {
    return Promise.resolve(config);
  }

  const isNvidia = String(config.vllmBaseUrl || "").includes("nvidia.com");
  const keyName = isNvidia ? "NVIDIA_API_KEY" : "VLLM_API_KEY";
  const keyLabel = isNvidia ? "NVIDIA" : "OpenAI 兼容";
  ...
  return new Promise((resolve) => {
    rl.question(`请输入您的 ${keyLabel} API Key: `, (apiKey) => {
      ...
      rl.question("是否将 API Key 保存到 .env 文件？(y/N): ", (save) => {
        ...
        rl.close();
        if (keyName === "NVIDIA_API_KEY") {
          config.nvidiaApiKey = apiKey.trim();
        } else {
...
async function main() {
  ...
  const displayKey = config.vllmApiKey || config.nvidiaApiKey;
  console.log(`   - API Key: ${displayKey ? "***" + displayKey.slice(-4) : "未设置"}`);
