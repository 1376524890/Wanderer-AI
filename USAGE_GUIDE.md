...
- `state/experience.md`：整场辩论经验总结
- `state/rl/rl_policy.json`：策略权重（战术概率 + 训练焦点）
- `state/rl/rl_metrics.json`：训练统计（平均奖励/平均分）
- `state/rl/rl_history.jsonl`：每轮策略更新记录
...
- 评委评分 → 转换为奖励（reward）
- 以移动基线（baseline）去噪，计算优势（advantage）
- 更新战术概率与训练焦点（policy update）
- 下一轮按策略动作对抗，形成自博弈闭环
...
```bash
# 查看策略权重与训练焦点
cat state/rl/rl_policy.json | jq

# 查看训练统计
cat state/rl/rl_metrics.json | jq

# 查看策略更新历史（最新10条）
tail -10 state/rl/rl_history.jsonl | jq
```
